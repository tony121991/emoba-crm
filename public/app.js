...
function renderUsers() {
  q('#usersList').innerHTML = state.users.length ? state.users.map(function (user) {
    ...
    return '<article class="user-card"><h3>' + user.name + '</h3><p>' + user.email + '</p><p>Rol: ' + user.role + ' · ' + (user.active ? 'Activo' : 'Inactivo') + '</p><p>' + (user.customerName || 'Sin cliente asignado') + '</p><p>' + caps + '</p></article>';
