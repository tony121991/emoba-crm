...
async function handleApi(req, res) {
  ...
  if (userMatch && method === 'DELETE') {
    if (!has(user, 'manage_users')) return json(res, 403, { error: 'No tienes permiso para eliminar usuarios' });
    const targetId = decodeURIComponent(userMatch[1]);
    if (targetId === user.id) return json(res, 400, { error: 'No puedes eliminar tu propio usuario mientras estas dentro' });
    ...
    if (target.role === 'admin' && target.active) {
      const adminCount = Number((await pool.query('SELECT COUNT(*) AS total FROM users WHERE role=$1 AND active=true AND id<>$2', ['admin', targetId])).rows[0].total);
      if (adminCount < 1) return json(res, 400, { error: 'Debe quedar al menos un administrador activo' });
    }
    const updated = await pool.query('UPDATE users SET active=false WHERE id=$1 RETURNING *', [targetId]);
    const removed = userFromRow(updated.rows[0]);
    await audit(user, 'eliminar_usuario', 'user', removed.id, { email: removed.email, role: removed.role });
    return json(res, 200, { user: sanitizeUser(removed) });
  }
